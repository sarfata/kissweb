...
Template.prototype.execute = function() {
  ...
  var match;
  while ((match = /<!([a-z]+) (.*?)\/>/.exec(text)) != null) {
    var replace = "";
    if (match[1] == "include") {
      if (match[2].substr(-3) == ".md") {
        replace = new MarkdownTemplate(this.includePath(match[2])).execute();
      }
      else {
      ...
      replace = new MarkdownTemplate(this.includePath(match[2]), true).execute();
    }
    else {
